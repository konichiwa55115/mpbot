...
CHOOSE_UR_AUDIO_MODE_BUTTONS = [
    ...
    [InlineKeyboardButton("دمج صوتيات / فيديوهات ",callback_data="audmerge")],
...
CHOOSE_UR_FILEMERGE_MODE = "اختر نوع ملفك "
CHOOSE_UR_FILEMERGE_MODE_BUTTONS = [
    [InlineKeyboardButton("صوتية",callback_data="mergefileaud")],
     [InlineKeyboardButton("فيديو ",callback_data="mergefilevid")]
]
CHOOSE_UR_AUDMERGE = "أرسل الصوتية التالية  \n تنبيه / بعد الانتهاء من إرسال الصوتيات اضغط دمج الآن "
CHOOSE_UR_AUDMERGE_BUTTONS = [
    [InlineKeyboardButton("دمج الآن ",callback_data="audmergenow")] ]
CHOOSE_UR_VIDMERGE = "أرسل الفيديو التالي  \n تنبيه / بعد الانتهاء من إرسال الفيديوهات اضغط دمج الآن "
CHOOSE_UR_VIDMERGE_BUTTONS = [
...
def callback_query(CLIENT,CallbackQuery):
  ...
  if CallbackQuery.data == "amplifyaud":
   ...
   CallbackQuery.edit_message_text(
    ...
    CallbackQuery.edit_message_text(
             text = CHOOSE_UR_FILEMERGE_MODE,
             reply_markup = InlineKeyboardMarkup(CHOOSE_UR_FILEMERGE_MODE_BUTTONS))
  elif CallbackQuery.data == "mergefileaud":
    cmd(f'''ffmpeg -i "{file_path}" -q:a 0 -map a "{mergdir}" -y ''')
    with open('list.txt','a') as f:
      f.write(f'''file '{mergdir}' \n''')
    CallbackQuery.edit_message_text(
             text = CHOOSE_UR_AUDMERGE,
             reply_markup = InlineKeyboardMarkup(CHOOSE_UR_AUDMERGE_BUTTONS))

  elif CallbackQuery.data == "audmergenow":
  ...
  elif CallbackQuery.data == "mergefilevid":
    with open('list.txt','a') as f:
      f.write(f'''file '{file_path}' \n''')
    CallbackQuery.edit_message_text(
             text = CHOOSE_UR_VIDMERGE,
             reply_markup = InlineKeyboardMarkup(CHOOSE_UR_VIDMERGE_BUTTONS))
  elif CallbackQuery.data == "vidmergenow":
    CallbackQuery.edit_message_text("جار الدمج")   
    cmd(f'''ffmpeg -f concat -safe 0 -i list.txt "{mp4file}" -y ''')
    with open(mp4file, 'rb') as f:
         bot.send_video(user_id, f)
    cmd(f'''rm list.txt && rm "{mp4file}" ''')
    shutil.rmtree('./downloads/') 
